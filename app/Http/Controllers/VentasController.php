...
class VentasController extends Controller
{
    ...
    public function localesPropios (Request $request){
        ...
        if(!empty($request)){
            ...
            $ventasPropios = DB::connection('sqlsrv2')->select("SET DATEFORMAT YMD EXEC SJ_VENTAS_LOCALES_PROPIOS ?, ?, ?", $datos);
    ...
    public function localesPropiosComp (Request $request){
        ...
        if(!empty($request)){
            ...
            $ventasPropiosComp = DB::connection('sqlsrv2')->select("SET DATEFORMAT YMD EXEC SJ_VENTAS_LOCALES_PROPIOS_COMP ?, ?, ?, ?, ?", $datos);
    ...
    public function localesFranquicias (Request $request){
        ...
        $franquiciasDisponibles = DB::connection('sqlsrv2')->select("SELECT NRO_SUCURSAL, DESC_SUCURSAL FROM FRANQUICIAS_LAKERS.DBO.SUCURSAL WHERE (NRO_SUCURSAL BETWEEN 803 AND 999) AND DESC_SUCURSAL NOT LIKE '%SOFIA%' ORDER BY NRO_SUCURSAL");
        
        if(!empty($request)){

            $datos = array($request->desde, $request->hasta, $request->suc);
            
            $ventasFranquicias = DB::connection('sqlsrv2')->select("SET DATEFORMAT YMD EXEC FRANQUICIAS_LAKERS.DBO.SJ_VENTAS_FRANQUICIAS ?, ?, ?", $datos);
    ...
    public function franquiciasComp (Request $request){
        ...
        if(!empty($request)){
            ...
            $datos = array($request->desde1, $request->hasta1, $request->desde2, $request->hasta2, $request->suc);
            $franquiciasComp = DB::connection('sqlsrv2')->select("SET DATEFORMAT YMD EXEC FRANQUICIAS_LAKERS.DBO.SJ_VENTAS_FRANQUICIAS_COMP ?, ?, ?, ?, ?", $datos);
    ...
    public function todos (Request $request){
        ...
        if(!empty($request)){
            ...
            $ventasTodos = DB::connection('sqlsrv2')->select("SET DATEFORMAT YMD EXEC SJ_VENTAS_LOCALES_TODOS ?, ?", $datos);
